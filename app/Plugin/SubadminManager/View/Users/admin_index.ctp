...
function validate()
{
	...
	if(ans=="0"){
		alert("Please select user to "+document.getElementById('action').value.toLowerCase());
		return false;
	}else{
		var answer = confirm('Are you sure you want to '+document.getElementById('action').value.toLowerCase()+' user(s)');
		if(!answer)
			return false;
	}
	return true;
