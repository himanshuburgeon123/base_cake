...
<script type='text/javascript'>
    $(function(){
      ...
      var lastScroll = 0;
      var loading_start = 0;
      var mail = <?=$this->paginator->counter('mail')?>;
      var mails = <?=$this->paginator->counter('mails')?>;
...
//alert(sta+'--'+st);
          ...
          //if((st > lastScroll) && ((doc_height-100) < (st+win_height)) ){
            ...
            if((scrollBottom <= scroll_value) && (mails >= (mail+1))){
                if(loading_start===0){
                    loading_start = 1;
                    mail++;
                    $('#loader_pagination').show();
                    $.ajax({
                        url:'<?=Router::url(array('plugin'=>'mail_manager','controller'=>'mails','action'=>'index',$search,'mail:'));?>'+mail,
                        async:false,
