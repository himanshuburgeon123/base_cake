...
<div>
    ...
    <fieldset>
        <dl>
            ...
            <dt>
                <label>You Tube <span style="color:red;">*</span></label>
            </dt>
            <dd>
                <?=$this->Form->text('youtube',array('class'=> 'small','size'=>'70','required'=>false)); ?>
...
<script type="text/javascript">
		$(document).ready(function(){
			$('#SettingAdminSocialForm').submit(function(){
				...
				$('#SettingAdminSocialForm > span#for_owner_cms').show();
				$('#SettingAdminSocialForm > button[type=submit]').attr({'disabled':true});
			   
				$.ajax({
					 url: '<?=Router::url(array('controller'=>'settings','action'=>'validation'))?>',
					async: false,
					data: data,
					dataType:'json', 
					type:'post',
				...
				if(status==0){
				   ...
				   $('#SettingAdminSocialForm > button[type=submit]').attr({'disabled':false});
				   $('#SettingAdminSocialForm > span#for_owner_cms').hide();
				}
